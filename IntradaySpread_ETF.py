...
for j in range(3):
    ...
    temp3=len(w.wsi(product[0],"close",start,\
              end,"periodstart=09:30:00;periodend=15:00:00").Times)
    data=pd.DataFrame(np.zeros((temp3,2)))
    for i in range(2):
        temp=w.wsi(product[2*(j)+i],"close",start,\
              end,"periodstart=09:30:00;periodend=15:00:00")
